...
var _iteratorNormalCompletion=true;var _didIteratorError=false;var _iteratorError=undefined;try{for(var _iterator=dependencies[Symbol.iterator](),_step;!(_iteratorNormalCompletion=(_step=_iterator.next()).done);_iteratorNormalCompletion=true){var dep=_step.value;var _buffer=unzip.decompress(dep);var fileInfo=json.files[dep];if(fileInfo.type==='Curve'&&fileInfo.dependencies){Module.resourcesMap[dep+'.efkcurve']=unzip.decompress(fileInfo.dependencies[0]);}else if(fileInfo.type==='Model'&&fileInfo.dependencies){Module.resourcesMap[dep]=unzip.decompress(fileInfo.dependencies[0]);}else{Module.resourcesMap[dep]=_buffer.buffer;}}}catch(err){_didIteratorError=true;_iteratorError=err;}finally{try{if(!_iteratorNormalCompletion&&_iterator.return){_iterator.return();}}finally{if(_didIteratorError){throw _iteratorError;}}}
...
},{key:"init",value:function init(webglContext,settings){this._gl=webglContext;this.contextStates=new ContextStates(this._gl);var instanceMaxCount=4000;var squareMaxCount=10000;var enableExtensionsByDefault=true;var enablePremultipliedAlpha=false;window.gl=this._gl;if(settings){if("instanceMaxCount"in settings){instanceMaxCount=settings.instanceMaxCount;}
...
if("enablePremultipliedAlpha"in settings){enablePremultipliedAlpha=settings.enablePremultipliedAlpha;}
if("enableTimerQuery"in settings&&settings.enableTimerQuery){window.ext_timer=window.gl.getExtension("EXT_disjoint_timer_query_webgl2");this._availableList=[];this._usingList=[];this._drawCount=0;this._accumulatedDrawTime=0;if("onTimerQueryReport"in settings){this._onTimerQueryReport=settings.onTimerQueryReport;}
if("timerQueryReportIntervalCount"in settings){this._timerQueryReportIntervalCount=settings.timerQueryReportIntervalCount;}else{this._timerQueryReportIntervalCount=300;}}}
this.ctx=Module.GL.registerContext(webglContext,{majorVersion:1,minorVersion:0,enableExtensionsByDefault:enableExtensionsByDefault});this._makeContextCurrent();this._restorationOfStatesFlag=true;this.contextStates.save();this.nativeptr=Core.Init(instanceMaxCount,squareMaxCount,enableExtensionsByDefault,enablePremultipliedAlpha);this.contextStates.restore();}
...
},{key:"draw",value:function draw(){var availableQuery=this._startQuery();this._makeContextCurrent();var program=null;if(this._restorationOfStatesFlag){program=this._gl.getParameter(this._gl.CURRENT_PROGRAM);this.contextStates.save();}else{this.contextStates.disableVAO();}
Core.Draw(this.nativeptr);if(this._restorationOfStatesFlag){this.contextStates.restore();this._gl.useProgram(program);}
this._endQuery(availableQuery);}},{key:"_startQuery",value:function _startQuery(){if(window.ext_timer!=null){var availableQuery=this._availableList.length?this._availableList.shift():this._gl.createQuery();this._gl.beginQuery(window.ext_timer.TIME_ELAPSED_EXT,availableQuery);return availableQuery;}}},{key:"_endQuery",value:function _endQuery(availableQuery){var _this=this;if(window.ext_timer!=null){this._gl.endQuery(window.ext_timer.TIME_ELAPSED_EXT);this._usingList.push(availableQuery);var disjoint=this._gl.getParameter(window.ext_timer.GPU_DISJOINT_EXT);if(disjoint){this._usingList.forEach(function(query){return _this._gl.deleteQuery(query);});}else{var usingQuery=this._usingList.length?this._usingList[0]:null;if(usingQuery){var resultAvailable=this._gl.getQueryParameter(usingQuery,this._gl.QUERY_RESULT_AVAILABLE);if(resultAvailable){var result=this._gl.getQueryParameter(usingQuery,this._gl.QUERY_RESULT);this._accumulatedDrawTime+=result;if(this._drawCount>=this._timerQueryReportIntervalCount){var averageDrawTime=this._accumulatedDrawTime/this._drawCount;this._drawCount=0;this._accumulatedDrawTime=0;if(this._onTimerQueryReport!=null){this._onTimerQueryReport(averageDrawTime);}}
this._drawCount++;this._availableList.push(this._usingList.shift());}}}}}},{key:"beginDraw",value:function beginDraw(){if(this._restorationOfStatesFlag){this.contextStates.save();}else{this.contextStates.disableVAO();}
...
return effect;}
},{key:"loadEffectPackage",value:function loadEffectPackage(data,Unzip){var scale=arguments.length>2&&arguments[2]!==undefined?arguments[2]:1.0;var onload=arguments[3];var onerror=arguments[4];if(Unzip==null)this._makeContextCurrent();var effect=new EffekseerEffect(this);effect.scale=scale;effect.onload=onload;effect.onerror=onerror;if(typeof data==="string"){var dirIndex=data.lastIndexOf("/");effect.baseDir=dirIndex>=0?data.slice(0,dirIndex+1):"";_loadBinFile(data,function(buffer){effect._loadFromPackage(buffer,Unzip);},effect.onerror);}else if(data instanceof ArrayBuffer){var buffer=data;effect._loadFromPackage(buffer,Unzip);}
return effect;}
